...
export default function App() {
    ...
    function setBase64Mp3ToPlayer(base64) {
        if (!base64)
            return;
    ...
    return (<div style={{ padding: 24, fontFamily: "system-ui, sans-serif", maxWidth: 960 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
            <div>
              ...
              <select style={{ width: "100%" }} value={targetLanguage} onChange={(e) => setTargetLanguage(e.target.value)}>
                {TARGET_LANG_OPTIONS.map((o) => (<option key={o.code} value={o.code}>
                    {o.label}
                  </option>))}
              </select>
            ...
            <div>
              ...
              <select style={{ width: "100%" }} value={nativeLanguage} onChange={(e) => setNativeLanguage(e.target.value)}>
                {NATIVE_LANG_OPTIONS.map((o) => (<option key={o.code} value={o.code}>
                    {o.label}
                  </option>))}
              </select>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
            <div>
              ...
              <select style={{ width: "100%" }} value={sttLanguage} onChange={(e) => setSttLanguage(e.target.value)}>
                {STT_LANG_OPTIONS.map((o) => (<option key={o.code} value={o.code}>
                    {o.label}
                  </option>))}
              </select>
        ...
        <div style={{ border: "1px solid #333", borderRadius: 12, padding: 12 }}>
          ...
          <textarea rows={4} style={{ width: "100%", fontSize: 16 }} placeholder="Enter a sentence (in target language)" value={userText} onChange={(e) => setUserText(e.target.value)}/>
          <button onClick={submitText} disabled={loading || !userText.trim()} style={{ marginTop: 10, padding: "8px 16px", fontSize: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={startRecording} disabled={loading || recState === "recording"} style={{ padding: "8px 16px" }}>
              🎙 Start
            </button>
            <button onClick={stopRecording} disabled={loading || recState !== "recording"} style={{ padding: "8px 16px" }}>
              ⏹ Stop
            </button>
            <button onClick={submitRecording} disabled={loading || !recordedBlob} style={{ padding: "8px 16px" }}>
              🚀 Send to teach
            </button>
      ...
      {error && (<pre style={{ color: "salmon", marginTop: 16, whiteSpace: "pre-wrap" }}>{error}</pre>)}

      {activeResult && (<div style={{ marginTop: 18, border: "1px solid #333", borderRadius: 12, padding: 12 }}>
          ...
          {assistantAudioUrl && (<div style={{ marginTop: 12 }}>
              <h4>🔊 Assistant audio (replayable)</h4>
              <audio ref={assistantAudioElRef} controls style={{ width: "100%" }} src={assistantAudioUrl}/>
